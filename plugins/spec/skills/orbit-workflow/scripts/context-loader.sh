...
def get_feature_state(feature_dir):
    """Get feature state from spec file frontmatter (case-insensitive)."""
    spec_path = find_spec_file(feature_dir)
    content = read_file(spec_path) if spec_path else None
...
if current_feature:
    ...
    if os.path.isdir(feature_dir):
        ...
        spec_path = find_spec_file(feature_dir)
        current_artifacts = {
            "spec": read_file(spec_path) if spec_path else None,
