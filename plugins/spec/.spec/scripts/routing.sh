#!/bin/bash
# Spec Command Routing Utilities
...
get_current_feature_dir() {
  ...
  if [ -f ".spec/state/current-session.md" ]; then
    local feature_id=$(grep "Feature ID:" ".spec/state/current-session.md" | cut -d':' -f2 | tr -d ' ' | head -1)
    if [ -n "$feature_id" ] && [ "$feature_id" != "N/A" ]; then
      # Find feature directory
      local feature_dir=$(find .spec/features -maxdepth 1 -name "*$feature_id*" -type d 2>/dev/null | head -1)
  ...
  local latest=$(ls -t .spec/features 2>/dev/null | head -1)
  if [ -n "$latest" ]; then
    echo ".spec/features/$latest"
  else
    echo "none"
  fi
}
...
route_flow_command() {
  ...
  case "$subcommand" in
    init)
      # Invoke spec:init skill
      echo "Routing to spec:init skill..."
      # Skill invocation handled by command wrapper
      ;;
    specify)
      echo "Routing to spec:specify skill..."
      # Skill invocation handled by command wrapper
      ;;
    plan)
      echo "Routing to spec:plan skill..."
      # Skill invocation handled by command wrapper
      ;;
    tasks)
      echo "Routing to spec:tasks skill..."
      # Skill invocation handled by command wrapper
      ;;
    implement)
      echo "Routing to spec:implement skill..."
      # Skill invocation handled by command wrapper
      ;;
    validate)
    ...
    *)
      ...
      echo ""
      echo "Run /spec for interactive menu"
      return 1
      ;;
  esac
}
...
render_flow_menu() {
  ...
  if [ "$current_phase" = "init" ]; then
    echo "  ➡️ init       - Initialize Spec (recommended next)"
  else
    echo "  ✅ init       - Initialize Spec (completed)"
  fi
  ...
  # Show suggestion
  if [ -n "$suggested" ]; then
    echo "Recommended: /spec $suggested"
    echo ""
  fi
}
