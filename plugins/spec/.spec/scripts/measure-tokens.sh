...
# Approximates token usage for Spec components
...
measure_commands() {
    ...
    for cmd in "$SPEC_ROOT/.claude/commands"/*.md; do
        if [[ -f "$cmd" ]]; then
...
measure_skills() {
    ...
    for skill_dir in "$SPEC_ROOT/.claude/skills"/*/; do
        if [[ -d "$skill_dir" ]]; then
...
measure_state() {
    ...
    if [[ -d "$SPEC_ROOT/.spec-state" ]]; then
        for file in "$SPEC_ROOT/.spec-state"/*.md; do
            if [[ -f "$file" ]]; then
...
measure_hooks() {
    ...
    if [[ -d "$SPEC_ROOT/.spec/hooks" ]]; then
        for file in "$SPEC_ROOT/.spec/hooks/core"/*.sh; do
            if [[ -f "$file" ]]; then
                local tokens=$(estimate_tokens "$file")
                total=$((total + tokens))
                printf "  %-30s %6d tokens\n" "$(basename "$file")" "$tokens"
...
main() {
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "Spec Token Usage Measurement"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
