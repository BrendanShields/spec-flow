...
# Configuration
HOOK_CONFIG="$SPEC_ROOT/.spec/hooks/config.json"
HOOK_LOG="$SPEC_ROOT/.spec/logs/hooks.log"
PERF_LOG="$SPEC_ROOT/.spec/logs/hook-performance.log"
...
#
# Usage: spec_run_hooks <event> <context_json>
#
# Arguments:
...
spec_run_hooks() {
  ...
  if [[ "$mode" == "async" ]]; then
    ...
    local hook_pid=$!
    echo "$hook_pid" > "$SPEC_ROOT/.spec/logs/hook-${event}.pid"
...
#   script: Hook script path (relative to .spec/hooks/)
...
_run_single_hook() {
  ...
  local hook_path="$SPEC_ROOT/.spec/hooks/$script"
  ...
  if jq -e '.activeFeature.id' <<<"$context" >/dev/null 2>&1; then
    ...
    sandbox_env="${sandbox_env} SPEC_FEATURE=$feature_id"
  fi
...
spec_hooks_enabled() {
  ...
  if [[ "${SPEC_HOOKS_ENABLED:-true}" == "false" ]]; then
    return 1
  fi
...
#
# Usage: spec_wait_for_hooks <event>
#
spec_wait_for_hooks() {
  local event=$1
  local pid_file="$SPEC_ROOT/.spec/logs/hook-${event}.pid"
...
export -f spec_run_hooks
export -f spec_hooks_enabled
export -f spec_wait_for_hooks
