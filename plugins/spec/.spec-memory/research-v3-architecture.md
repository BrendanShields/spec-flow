# Spec v3.0 Architecture Research

**Research Date**: 2024-10-31
**Feature**: 002-spec-consolidation-v3
**Researcher**: Claude (spec-researcher agent)
**Status**: Complete
...
This research document provides architecture decisions for Spec v3.0, covering:
...
Spec v2.1 has 8 separate commands (`/spec-init`, `/spec-specify`, etc.). Users must remember multiple entry points, resulting in cognitive overhead and increased documentation burden.

**Requirements**:
- Single `/spec` entry point
- Route to appropriate skills based on subcommand or context
- Support auto-detection (e.g., `/spec "Feature"` → `spec:specify`)
...
case "$1" in
    ...
    specify|s|"")  # Empty triggers auto-detect
        invoke_skill "spec:specify" "$@"
        ;;
    status|st)
        invoke_skill "spec:status" "$@"
        ;;
    *)
...
# Hub command: .claude/commands/spec.md
command: spec
description: Unified Spec workflow assistant
...
spec_router() {
    ...
    case "$subcommand" in
        # Workflow commands
        init)           invoke_skill "spec:init" "$@" ;;
        specify|spec)   invoke_skill "spec:specify" "$@" ;;
        clarify)        invoke_skill "spec:clarify" "$@" ;;
        plan)           invoke_skill "spec:plan" "$@" ;;
        tasks)          invoke_skill "spec:tasks" "$@" ;;
        implement|impl) invoke_skill "spec:implement" "$@" ;;

        # Utility commands
        status|st)      invoke_skill "spec:status" "$@" ;;
        validate|check) invoke_skill "spec:validate" "$@" ;;
        help|h|-h|--help) show_contextual_help "$@" ;;

        # Team commands
        assign)         invoke_skill "spec:assign" "$@" ;;
        locks)          invoke_skill "spec:locks" "$@" ;;
        unlock)         invoke_skill "spec:unlock" "$@" ;;

        # Master spec
        sync)           invoke_skill "spec:sync" "$@" ;;
        ...
        *)
            if [[ "$subcommand" =~ ^[A-Z] ]]; then
                ...
                invoke_skill "spec:specify" "$subcommand $@"
            else
...
invoke_skill() {
    ...
    source "${SPEC_ROOT}/.claude/skills/${skill}/SKILL.md"
...
auto_detect_workflow_step() {
    local session_state="$HOME/.spec-state/session.md"

    if [[ ! -f "$session_state" ]]; then
        echo "No Spec project initialized. Run '/spec init'"
        return 1
    fi
    ...
    case "$current_phase" in
        specification)  invoke_skill "spec:plan" ;;
        planning)       invoke_skill "spec:tasks" ;;
        implementation) invoke_skill "spec:implement" ;;
        *)              invoke_skill "spec:status" ;;
    esac
}
```
...
**Tasks**:
- [ ] Create `.claude/commands/spec.md` (4h)
...
Spec v2.1 loads ~88,700 tokens per invocation. With 14 skills, many are loaded but never used in a single session. Target: 80% token reduction (17,740 tokens).
...
```markdown
<!-- .claude/skills/spec-specify/SKILL.md -->
# Spec: Specify
...
/spec specify --examples

# System loads examples.md
source .claude/skills/spec-specify/examples.md
```
...
source .claude/skills/spec-specify/reference.md
```
...
- SPEC_SPEC_TEMPLATE: Custom template path
- SPEC_AUTO_RESEARCH: Enable/disable auto-research

## Integration Points
- Invokes: spec-researcher agent
- Outputs: features/###-name/spec.md
- Updates: .spec-memory/progress.json
```
...
Command:  /spec-specify.md          2,800 tokens
Skill:    spec:specify/SKILL.md     3,700 tokens
Agent:    spec-researcher           3,300 tokens
...
Command:  /spec (hub)                 750 tokens
Skill:    spec:specify (core only)  1,200 tokens
...
spec-skill-server --skill=specify &
...
Spec v2.1 uses markdown files for state:
- `.spec-state/current-session.md` (183 lines, git-ignored)
- `.spec-memory/*.md` (markdown tables and lists)
...
```bash
# .gitattributes
.spec-memory/progress.json merge=spec-json
.spec-memory/decisions.json merge=spec-json

# .git/config
[merge "spec-json"]
    name = Spec JSON merge driver
    driver = spec-merge-json %O %A %B

# Script: spec-merge-json
#!/usr/bin/env bash
...
Spec v3.0 consolidates 8 commands into 1. Existing users have:
- Scripts referencing `/spec-specify`, `/spec-plan`, etc.
...
```bash
# /spec-specify (wrapper)
#!/usr/bin/env bash
echo "⚠️  Warning: /spec-specify is deprecated. Use '/spec specify' instead."
exec /spec specify "$@"
```
...
```bash
# .claude/commands/spec-specify.md (v3.0 wrapper)
#!/usr/bin/env bash

# Deprecation warning (once per session)
if [[ ! -f "/tmp/.spec-v3-warning-shown" ]]; then
    cat >&2 <<EOF
⚠️  DEPRECATION WARNING

The command '/spec-specify' is deprecated and will be removed in v4.0 (2025-10-31).

Please update to:
  /spec specify [args]

For more info: /spec help migrate

This warning will be shown once per session.
EOF
    touch "/tmp/.spec-v3-warning-shown"
fi
...
─────────────────────────  ─────────────────────────
/spec-init              /spec init
/spec-specify "Feature" /spec "Feature"
/spec-plan              /spec plan
...
#### Auto-Migration Tool
```bash
# /spec migrate-scripts /path/to/repo
migrate_scripts() {
    local path="${1:-.}"

    echo "Searching for deprecated Spec commands in $path..."
    ...
    while IFS= read -r file; do
        ...
        if grep -q "/spec-" "$file"; then
            ...
            # Replace commands
            sed -i '' \
                -e 's|/spec-init|/spec init|g' \
                -e 's|/spec-specify|/spec specify|g' \
                -e 's|/spec-plan|/spec plan|g' \
                -e 's|/spec-tasks|/spec tasks|g' \
                -e 's|/spec-implement|/spec implement|g' \
                -e 's|/spec-status|/spec status|g' \
                -e 's|/spec-validate|/spec validate|g' \
                -e 's|/spec-help|/spec help|g' \
                "$file"
...
- New /spec command fully functional
...
alias /spec-specify='/spec specify'
```
...
**Dependencies**: ADR-001 (new /spec command must exist)
...
#### Lock Acquisition
```bash
# spec-lock-acquire <feature> <task>
acquire_lock() {
    local feature="$1"
    local task="$2"

    local lock_file=".spec/locks/${feature}-${task}.lock"
    local lock_dir=".spec/locks"
    ...
    if [[ -f "$lock_file" ]]; then
        ...
        # Check if process still running
            ...
            echo "   To override: /spec unlock $task --force"
            return 1
        fi
    fi
...
#### Lock Release
```bash
# spec-lock-release <feature> <task>
release_lock() {
    local feature="$1"
    local task="$2"
...
implement_task() {
    ...
    invoke_skill "spec:implement" --task="$task"
...
Spec v2.1 creates individual specs per feature (`features/###-name/spec.md`). Teams need:
...
> **⚠️ DO NOT EDIT DIRECTLY** - Edit source files and run `/spec sync master-spec`
...
[Full ADR](.spec-memory/adr-002.md)
...
*Generated by Spec v3.0 Master Spec Generator*
...
> **⚠️ DO NOT EDIT** - Run \`/spec sync master-spec\` to regenerate
...
EOF
    ...
    echo "" >> "$temp_spec"
    if [[ -f ".spec/config/product-requirements.md" ]]; then
        # Extract vision section
        sed -n '/^## Vision/,/^##/p' .spec/config/product-requirements.md | head -n -1 >> "$temp_spec"
    fi
    echo "" >> "$temp_spec"
    ...
    echo "" >> "$temp_spec"
    if [[ -f ".spec/config/architecture-blueprint.md" ]]; then
        # Extract overview
        sed -n '/^## Overview/,/^##/p' .spec/config/architecture-blueprint.md | head -n -1 >> "$temp_spec"
    fi
    echo "" >> "$temp_spec"
...
smart_sync_master_spec() {
    local master_spec=".spec/master-spec.md"
    local cache_file=".spec/.master-spec-cache.json"

    # Track source file timestamps
    local sources=(
        ".spec/config/product-requirements.md"
        ".spec/config/architecture-blueprint.md"
        ".spec-memory/progress.json"
        ".spec-memory/decisions.json"
        features/*/spec.md
    )
...
if echo "$changed_files" | grep -q "features/.*spec.md"; then
    ...
    /spec sync master-spec --quiet

    # Auto-commit master spec
    git add .spec/master-spec.md
...
Command: /spec version bump [major|minor|patch]
```
...
- [Spec v2.1 Architecture](../docs/ARCHITECTURE.md)
...
*Researcher: Claude (spec-researcher)*
