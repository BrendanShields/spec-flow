...
git add .spec/
git add .spec-state/current-session-template.md
git add .spec-memory/*template*

# Commit
git commit -m "feat: Add Spec core structure and configuration
...
# Commit
git commit -m "feat: Add Spec skills, commands, and integrations
...
# Commit
git commit -m "docs: Update documentation for Spec v2.1.0
...
git add features/
git add .spec-state/current-session.md
git add .spec-memory/*.md
git add ../../scripts/

# Commit
git commit -m "feat: Initialize Spec workflow for plugin development
...
```bash
# Push branch
git push -u origin feat/spec-refactor-stabilization

# Create PR
gh pr create \
  --title "feat: Spec v2.1.0 - Complete plugin refactor" \
  --body "## Summary
- Complete Navi→Spec migration
...
- ✨ Added optimized Spec implementation
...
**Dependencies**: T021
```bash
git tag -a v2.1.0 -m "Spec v2.1.0 - Specification-driven development"
...
```bash
# Compress backup
tar -czf ../spec-backup-$(date +%Y%m%d).tar.gz ../spec-backup-*/
