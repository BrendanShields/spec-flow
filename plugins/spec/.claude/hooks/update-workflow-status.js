 ...
 * Maintains state in .spec/.state.json showing current phase, progress percentage,
...
const STATE_FILE = '.spec/.state.json';

// Skill completion tracking
const SKILL_WORKFLOW = {
  'spec:init': { phase: 'setup', progress: 10 },
  'spec:blueprint': { phase: 'setup', progress: 20 },
  'spec:specify': { phase: 'specification', progress: 30 },
  'spec:clarify': { phase: 'specification', progress: 40 },
  'spec:plan': { phase: 'planning', progress: 50 },
  'spec:analyze': { phase: 'planning', progress: 60 },
  'spec:tasks': { phase: 'planning', progress: 70 },
  'spec:checklist': { phase: 'validation', progress: 80 },
  'spec:implement': { phase: 'implementation', progress: 90 },
  'spec:update': { phase: 'maintenance', progress: 100 }
};
...
function getNextStep(lastSkill) {
  const workflow = {
    ...
    'spec:specify': 'spec:clarify or spec:plan',
    'spec:clarify': 'spec:plan',
    'spec:plan': 'spec:tasks',
    'spec:tasks': 'spec:analyze or spec:implement',
    'spec:analyze': 'spec:implement',
    'spec:checklist': 'spec:implement'
  };
