 ...
 *
 * @async
 * @param {string} skillName - Name of the Flow skill being executed (e.g., 'spec:plan')
 ...
 *
 * @example
 * const result = await validatePrerequisites('spec:plan', { workingDir: '/app' });
 * if (!result.valid) {
...
function getSuggestion(skillName, missing) {
  const suggestions = {
    'spec:plan': 'Run spec:specify first to create a specification',
    'spec:tasks': 'Run spec:plan first to create technical plan',
    'spec:implement': 'Run spec:tasks first to generate task list',
    'spec:analyze': 'Create specification and plan first',
    'spec:clarify': 'Run spec:specify to create initial specification'
  };
