 ...
 *
 * Executes after spec:specify to trigger follow-up actions and integrations.
...
module.exports = async function postSpecify(context) {
  ...
  if (clarificationNeeded.count > 0 && config.workflow.requireClarification) {
    ...
    context.messages.push({
      type: 'info',
      text: `Specification created with ${clarificationNeeded.count} clarifications needed. Auto-triggering spec:clarify...`
    });
  } else if (clarificationNeeded.count > 0) {
    context.messages.push({
      type: 'suggestion',
      text: `Specification has ${clarificationNeeded.count} clarification markers. Consider running spec:clarify to resolve ambiguities.`
    });
  }
...
async function updateProjectIndex(specPath, metadata) {
  const indexPath = '.spec/project-index.json';
...
async function recordTelemetry(telemetryData) {
  ...
  const fs = require('fs').promises;
  const telemetryPath = '.spec/telemetry.jsonl';
