 ...
 *
 * Executes before spec:specify to prepare environment and gather context.
  ...
  if (config.workflow.requireBlueprint && !options.skipValidation) {
    ...
    if (!hasBlueprint) {
      context.warnings.push({
        type: 'warning',
        text: 'Architecture blueprint not found. Consider running spec:blueprint to define architecture standards.'
      });
    }
  }
...
async function loadDomainTemplates(domain) {
  ...
  const templatePath = `plugins/spec/templates/domains/${domain}`;
