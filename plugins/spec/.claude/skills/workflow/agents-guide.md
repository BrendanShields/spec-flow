...
1. **spec:implement** → **spec-implementer**: When tasks.md has 3+ tasks OR parallel execution requested
2. **spec:plan** → **spec-researcher**: When technology decisions needed OR domain patterns unknown
3. **spec:analyze** → **spec-analyzer**: When validation spans 3+ artifacts OR deep consistency check needed
...
- **Configuration**: CLAUDE.md settings, .spec/config.json
...
### Artifacts
- spec.md: /path/to/features/003-user-authentication/spec.md
- plan.md: /path/to/features/003-user-authentication/plan.md
- tasks.md: /path/to/features/003-user-authentication/tasks.md
...
- `implementation-context.md` (generated by spec:implement)
...
- `plan.md` (from feature directory)
  ...
  - API contracts
  - Testing strategy
- `.spec-state/current-session.md` (session state)
...
The agent updates `.spec-state/current-session.md` in real-time:
...
**Configuration** (in CLAUDE.md or .spec/config.json):
```markdown
...
**Invoked By**: `spec:plan` skill (automatically during planning phase)
...
- `.spec/architecture-blueprint.md` (if exists)
  - Existing tech stack constraints
  - Architectural principles
  - Approved technologies
- `.spec-memory/DECISIONS-LOG.md` (existing ADRs)
  - Previous architecture decisions
  - Technology choices made
  - Rationale and trade-offs
- `research-query.md` (generated by spec:plan)
...
1. **Research Report** (`.spec/research/reports/[topic].md`):
```markdown
...
2. **Architecture Decision Record** (appended to `.spec-memory/DECISIONS-LOG.md`):
```markdown
...
**References**:
- Research: `.spec/research/reports/authentication-library.md`
- Passport docs: https://www.passportjs.org/
```

3. **Quick Summary** (returned to spec:plan):
```markdown
...
- spec:plan detects decision points
...
**Invoked By**: `spec:analyze` skill

**When Invoked**:
- User runs `/spec analyze` (manual validation)
- After spec:update modifies requirements
- Before spec:implement starts (pre-flight check)
- During spec:orchestrate workflow (automatic checkpoints)
...
- `.spec/architecture-blueprint.md` (architecture standards)
- `.spec-memory/DECISIONS-LOG.md` (ADR history)
- `validation-config.md` (generated by spec:analyze)
...
The agent produces a validation report (`.spec-state/validation-report.md`):
...
**Best Practices**:
- Run after spec:update (catch inconsistencies early)
- Run before spec:implement (pre-flight check)
...
|-------|-------------|-----------|------------|
| `spec:init` | ❌ Never | Simple directory creation | None |
| `spec:discover` | ✅ Sometimes | Large codebase (500+ files) | spec-analyzer (brownfield mode) |
| `spec:blueprint` | ❌ Never | Template generation | None |
| `spec:generate` | ❌ Never | Template-based spec creation | None |
| `spec:clarify` | ❌ Never | Interactive Q&A | None |
| `spec:checklist` | ❌ Never | Checklist generation | None |
| `spec:plan` | ✅ Always | Technology decisions needed | spec-researcher |
| `spec:analyze` | ✅ Sometimes | 3+ artifacts to validate | spec-analyzer |
| `spec:tasks` | ❌ Never | Task breakdown from plan | None |
| `spec:implement` | ✅ Usually | 3+ tasks OR --parallel flag | spec-implementer |
| `spec:update` | ❌ Never | Direct file editing | None |
| `spec:metrics` | ❌ Never | State file aggregation | None |
| `spec:orchestrate` | ✅ Transitively | Uses plan/analyze/implement | All agents (as needed) |
...
**Per-Feature Configuration** (in `.spec/config.json`):
...
**spec:plan flags** (researcher configuration):
```bash
...
2. **Skill-Level Graceful Degradation**:
   ...
   - Log degraded mode to `.spec-state/warnings.log`
...
Checkpoint saved to .spec-state/checkpoints/2025-11-02-14-30.md

# User recovery
/spec status                # Check current state
# Review T005 error in .spec-state/implementation-errors.log
...
# View agent logs
tail -f .spec-state/agent-execution.log
...
cat .spec-state/implementation-context.md
...
# Review checkpoint
cat .spec-state/checkpoints/latest.md
...
cat .spec-state/agent-execution.log
...
|-------|---------|------------|-------------|
| **spec-implementer** | Execute tasks in parallel with progress tracking | spec:implement | Parallel execution, dependency resolution |
| **spec-researcher** | Research best practices and document decisions | spec:plan | WebSearch, ADR generation |
| **spec-analyzer** | Validate consistency across artifacts | spec:analyze | Deep cross-artifact validation |
...
**Control Points**:
- Configuration files (CLAUDE.md, .spec/config.json)
