#!/bin/bash
# Specter Command Routing Utilities
...
get_current_feature_dir() {
  ...
  if [ -f ".specter/state/current-session.md" ]; then
    local feature_id=$(grep "Feature ID:" ".specter/state/current-session.md" | cut -d':' -f2 | tr -d ' ' | head -1)
    if [ -n "$feature_id" ] && [ "$feature_id" != "N/A" ]; then
      # Find feature directory
      local feature_dir=$(find .specter/features -maxdepth 1 -name "*$feature_id*" -type d 2>/dev/null | head -1)
  ...
  local latest=$(ls -t .specter/features 2>/dev/null | head -1)
  if [ -n "$latest" ]; then
    echo ".specter/features/$latest"
  else
    echo "none"
  fi
}
...
route_flow_command() {
  ...
  case "$subcommand" in
    init)
      ...
      echo "Routing to specter:init skill..."
      # Skill invocation handled by command wrapper
      ;;
    specify)
      echo "Routing to specter:specify skill..."
      # Skill invocation handled by command wrapper
      ;;
    plan)
      echo "Routing to specter:plan skill..."
      # Skill invocation handled by command wrapper
      ;;
    tasks)
      echo "Routing to specter:tasks skill..."
      # Skill invocation handled by command wrapper
      ;;
    implement)
      echo "Routing to specter:implement skill..."
      # Skill invocation handled by command wrapper
      ;;
    validate)
    ...
    *)
      ...
      echo ""
      echo "Run /specter for interactive menu"
      return 1
      ;;
  esac
}
...
render_flow_menu() {
  ...
  if [ "$current_phase" = "init" ]; then
    echo "  ➡️ init       - Initialize Specter (recommended next)"
  else
    echo "  ✅ init       - Initialize Specter (completed)"
  fi
  ...
  # Show suggestion
  if [ -n "$suggested" ]; then
    echo "Recommended: /specter $suggested"
    echo ""
  fi
}
