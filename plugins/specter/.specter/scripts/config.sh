...
# Manages JSON configuration at .specter/config/flow.json

# Initialize config with defaults
init_flow_config() {
  local type="${1:-brownfield}"
  local config_dir=".specter/config"
...
get_flow_config() {
  local key="$1"
  local config_file=".specter/config/flow.json"
...
should_prompt_interactive() {
  ...
  # Check global flag
  if [ "$SPECTER_NO_INTERACTIVE" = "true" ]; then
    return 1  # Skip prompts
  fi
