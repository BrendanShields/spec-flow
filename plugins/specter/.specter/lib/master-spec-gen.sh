...
#
# Usage: specter_generate_master_spec
#
specter_generate_master_spec() {
  local master_spec="$SPECTER_ROOT/.specter/master-spec.md"
  local cache_dir="$SPECTER_ROOT/.specter/cache/master-spec"
  ...
  local features_dir="$SPECTER_ROOT/features"
...
**Version**: $(cat "$SPECTER_ROOT/.specter/config/version" 2>/dev/null || echo "3.1.0")
...
EOF
  ...
  # Product Vision
  if [[ -f "$SPECTER_ROOT/.specter/config/product-requirements.md" ]]; then
    cat >> "$temp_spec" <<EOF
## Product Vision

$(cat "$SPECTER_ROOT/.specter/config/product-requirements.md")
...
EOF
  ...
  # Architecture
  if [[ -f "$SPECTER_ROOT/.specter/config/architecture-blueprint.md" ]]; then
    cat >> "$temp_spec" <<EOF
## Architecture

$(cat "$SPECTER_ROOT/.specter/config/architecture-blueprint.md")
...
EOF
  ...
  if [[ -f "$SPECTER_ROOT/.specter-memory/CHANGES-PLANNED.md" ]]; then
    cat "$SPECTER_ROOT/.specter-memory/CHANGES-PLANNED.md" >> "$temp_spec"
  else
...
EOF
  ...
  if [[ -f "$SPECTER_ROOT/.specter-memory/DECISIONS-LOG.md" ]]; then
    cat "$SPECTER_ROOT/.specter-memory/DECISIONS-LOG.md" >> "$temp_spec"
  else
...
EOF
  ...
  if [[ -f "$SPECTER_ROOT/.specter-memory/WORKFLOW-PROGRESS.md" ]]; then
    cat "$SPECTER_ROOT/.specter-memory/WORKFLOW-PROGRESS.md" >> "$temp_spec"
  else
...
*Auto-generated by Specter v$(cat "$SPECTER_ROOT/.specter/config/version" 2>/dev/null || echo "3.1.0")*
...
_generate_feature_section() {
  ...
  if [[ -f "$SPECTER_ROOT/.specter-state/session.json" ]]; then
    status=$(jq -r ".features[]? | select(.id == \"$feature_id\") | .status // \"active\"" \
      "$SPECTER_ROOT/.specter-state/session.json" 2>/dev/null || echo "active")
  fi
...
_is_feature_active() {
  ...
  local features_dir="$SPECTER_ROOT/features/$feature_id"
...
# Export function
export -f specter_generate_master_spec
