...
#
# Usage: specter_update_state <state_file> <jq_expression>
#
# Arguments:
...
#
# Example:
#   specter_update_state .specter-state/session.json '.activeFeature.phase = "planning"'
#
specter_update_state() {
  ...
  if [[ ! "$state_file" = /* ]]; then
    state_file="$SPECTER_ROOT/$state_file"
  fi
  ...
  if specter_generate_md "$state_file" > "${md_file}.tmp" 2>/dev/null; then
...
#
# Usage: specter_generate_md <json_file>
#
# Arguments:
...
specter_generate_md() {
  ...
  # Convert relative to absolute path
  if [[ ! "$json_file" = /* ]]; then
    json_file="$SPECTER_ROOT/$json_file"
  fi
...
*Auto-generated by Specter from session.json*
...
#
# Usage: specter_generate_md_if_changed <json_file>
#
...
#
# Usage: specter_validate_state <state_file>
#
# Returns:
#   0 if valid, 1 if invalid
#
specter_validate_state() {
  local state_file=$1
  ...
  if [[ -f "$md_file" ]]; then
    ...
    specter_generate_md "$state_file" > "$temp_md" 2>/dev/null
...
export -f specter_update_state
export -f specter_generate_md
export -f specter_generate_md_if_changed
export -f specter_validate_state
