...
SPECTER_DIR="${PWD}/.specter"
FEATURES_DIR="${PWD}/features"
MEMORY_DIR="${PWD}/.specter-memory"
OUTPUT_FILE="${SPECTER_DIR}/master-spec.md"
...
**Version**: $(cat "${SPECTER_DIR}/version" 2>/dev/null || echo "3.0.0")
...
EOF
    ...
    echo "" >> "$temp_file"
    if [[ -f "${SPECTER_DIR}/product-requirements.md" ]]; then
        cat "${SPECTER_DIR}/product-requirements.md" >> "$temp_file"
    else
    ...
    echo "" >> "$temp_file"
    if [[ -f "${SPECTER_DIR}/architecture-blueprint.md" ]]; then
        cat "${SPECTER_DIR}/architecture-blueprint.md" >> "$temp_file"
    else
