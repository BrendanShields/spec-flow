...
## Scope Control
SPECTER_DISCOVER_INCLUDE_TESTS=true      # Include test directories
SPECTER_DISCOVER_ANALYZE_GIT=true        # Analyze git history
SPECTER_DISCOVER_DETECT_HOTSPOTS=true    # Calculate hotspot risk

## Path Exclusions
SPECTER_DISCOVER_EXCLUDE_PATHS=node_modules,dist,build,.git,vendor

## Performance
SPECTER_DISCOVER_MAX_FILE_SIZE=1048576   # Skip files >1MB
SPECTER_DISCOVER_PARALLEL_WORKERS=4      # Parallel processing
SPECTER_DISCOVER_TIMEOUT_SECONDS=600     # 10 minute timeout

## JIRA Integration
SPECTER_ATLASSIAN_SYNC=enabled           # Enable JIRA analysis
SPECTER_JIRA_PROJECT_KEY=PROJ            # Default project key
SPECTER_DISCOVER_SPRINT_LOOKBACK=6       # Number of sprints

## Report Format
SPECTER_DISCOVER_OUTPUT_FORMAT=markdown  # markdown|json|both
SPECTER_DISCOVER_REPORT_DIR=discovery    # Output directory
```
...
const CACHE_DIR = '.specter-cache/discover';
...
Discovery updates `.specter-memory/WORKFLOW-PROGRESS.md`:
...
Create `.specter/scripts/custom-discovery.sh`:
...
Add custom patterns to `.specter/config/discovery-patterns.json`:
