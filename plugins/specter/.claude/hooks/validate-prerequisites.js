 ...
 *
 * @async
 * @param {string} skillName - Name of the Flow skill being executed (e.g., 'specter:plan')
 ...
 *
 * @example
 * const result = await validatePrerequisites('specter:plan', { workingDir: '/app' });
 * if (!result.valid) {
...
function getSuggestion(skillName, missing) {
  const suggestions = {
    'specter:plan': 'Run specter:specify first to create a specification',
    'specter:tasks': 'Run specter:plan first to create technical plan',
    'specter:implement': 'Run specter:tasks first to generate task list',
    'specter:analyze': 'Create specification and plan first',
    'specter:clarify': 'Run specter:specify to create initial specification'
  };
