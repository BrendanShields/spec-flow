 ...
 *
 * Analyzes user prompts using pattern matching to suggest appropriate Specter skills.
...
// Intent patterns mapped to Specter skills
 ...
 *
 * Analyzes prompt against predefined patterns to identify which Specter skills
 ...
 * // Returns: { detectedIntents: ['specter:specify'], confidence: 'high', ... }
 */
async function detectIntent(prompt) {
  ...
  if (CONTEXT_HINTS.hasUserStory.test(prompt) && !detectedIntents.includes('specter:specify')) {
    suggestions.push({
      skill: 'specter:specify',
...
function getNextStep(lastCompleted) {
  const workflow = {
    ...
    'specter:specify': 'specter:clarify',
