...
import { FileChange } from '../types/specter.js';
...
export class SpecterFileWatcher extends EventEmitter {
  ...
  start(): void {
    ...
    this.watchPaths = [
      path.join(this.projectRoot, '.specter-state', '**/*.md'),
      path.join(this.projectRoot, '.specter-memory', '**/*.md'),
      path.join(this.projectRoot, 'features', '**/*.md'),
      path.join(this.projectRoot, '.specter', '**/*'),
    ];
  ...
  private handleFileChange(
    ...
    const timer = setTimeout(() => {
      ...
      if (relativePath.includes('.specter-state')) {
        this.emit('state-change', change);
      } else if (relativePath.includes('.specter-memory')) {
