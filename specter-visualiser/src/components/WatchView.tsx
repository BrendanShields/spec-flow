...
import { SpecterFileWatcher } from '../services/fileWatcher.js';
import { SpecterParser } from '../services/specterParser.js';
import { FileChange, SessionState, WorkflowProgress } from '../types/specter.js';
...
export const WatchView: React.FC<WatchViewProps> = ({ projectRoot, verbose }) => {
  ...
  const watcherRef = useRef<SpecterFileWatcher | null>(null);
  const parserRef = useRef<SpecterParser>(new SpecterParser(projectRoot));
  ...
  const startWatching = () => {
    const watcher = new SpecterFileWatcher(projectRoot);
